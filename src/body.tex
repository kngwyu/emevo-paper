...
We designed a continuous 2D environment shown in \Cref{figure:env}. Blue circles indicate agents, and red circles indicate foods. The environment is implemented by a 2D rigid-body physics simulation, where agents can move by producing driving forces on the left and right sides of the body.
...
Letting $n^{t}_{i}$ be the number of foods that an agent $i$ ate at time $t$, and let $a^{t}_{i}$ represent the action that the agent took at that time, we model the reward of
