...
\citet{suttonReinforcementLearningIntroduction2018}. MDP $\M{}$ consists of a tuple $(\X{}, \A{}, p, r, \gamma)$, where $\X{}$ is the finite set of states, $\A{}$ is the finite set of actions, $p: \X\times\A\times \X \rightarrow [0, 1]$ is the state transition function, $r: \X\times\A \rightarrow [r_{\min}, r_{\max} ]$ is the reward function, where $r_{\min}, r_{\max}$ are minimum and maximum. $\gamma$ is the discount factor introduced to define a mathematically-well behaved objective. I use $S_t$ and $A_t$ for denoting random variables of the state and action experienced at time $t$. The objective function of an MDP is the discounted cumulative return $G \defequal{} \sum_{t=0}^{\infty}\gamma^t R_{t}$, where $R_t = r(S_t, A_t)$ is the reward received at time $t$. In RL, an agent aims to maximize the expected discounted return $\E \left[G|\pi\right]$ by finding the optimal policy $\pi^{*}$. Thanks to the Markovian assumption, it is sufficient to consider a memoryless policy $\pi: \X \times \A \rightarrow [0, 1]$ as an agent's action strategy. $\pi$ induces two value functions: state-value function $V^\pi(s) \defequal \sum_{a \in \A} \pi(a|s) \left( r(s, a) + \gamma \sum_{s' \in \X} P(s'|s, a) V^\pi(s') \right)$ and action-value function $Q^\pi(s, a) \defequal r(s, a) + \gamma \sum_{s' \in \X} P(s'|s, a) V^\pi(s')$. Optimal policy maximizes $V^{\pi^{*}}(s)$ in all reachable states, so an agent's goal is almost equal to maximize $V^{\pi}$. Many representative RL methods like Q-learning \citep{watkinsTechnicalNoteQLearning1992} utilize this nature of RL problems and try to maximize $Q$ or $V$.

The concept of reward in our research is influenced by the idea of reward systems in neuroscience research \citep{schultzNeuronalRewardDecision2015, berridgePleasureSystemsBrain2015}. Notably, \citet{berridgeDissectingComponentsReward2009} argue that brain reward system consists of three independent components: liking, wanting, and associative learning. In analogy with computational RL, liking
