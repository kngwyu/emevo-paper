...
\Cref{figure:result-baseline} shows the reward weights of the last $5000$ agents with 5 random seeds in the baseline environment, where the x-axis shows $w_{\mathrm{food}}$ and the y-axis shows $w_{\mathrm{act}}$. First of all, compared to the result by random walk in \Cref{figure:rew-random}, the resulting reward weight are clearly biased, so we conclude that evolution happend by our simulation. Food rewards are almost positive in all 5 runs; the majority exceeds $4$. Thus, we can say that agents have acquired naturalistic positive food rewards only from birth and death rules.
