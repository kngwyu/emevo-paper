...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \ForAll{$agent \in Pop$}
      ...
      \EndOnce{}
      \State{Update $agent$'s energy level $e$ based on $a$, eaten food, and whether it did RL}
    \EndFor{}
    ...
    \ForAll{$agent \in Pop$ whose energy level is $e$ and age is $t$}
      \If{$H \sim h(s, e)$ and $H=1$}
        \State{$agent$ is removed from $Pop$ and $Env$}
        \Comment{agent is dead}
      \ElsIf{$B \sim b(e)$, $B=1$, and $Env$ has space to place a new agent}
