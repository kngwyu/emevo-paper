...
\begin{figure}[!htb]
  ...
  \caption{
    Comparison of the standard deviation of the number of children between different food locations, computed for the population at each time step.
...
\begin{figure}[t]
  ...
  \caption{
    Visualization of life history of agents in the environment with poor foods.
    \textbf{Upper:} The population of agents with positive or negative rewards for poor foods.
    \textbf{Lower:} Life history of agents in the red period in the upper figure. Lines indicate the trajectories, and the circle indicates places where agents got their children. The radius of the circle corrensponds to the number of grand children that the newborn produced.
  }\label{figure:lh-poor}
\end{figure}

\Cref{figure:result-poor} shows the results with poor foods. While rewards for normal foods are consistently positive in all five runs, the evolution of rewards for poor foods is unstable. In \cref{figure:lh-poor}, we show the lifehistory of agents in a selected period with almost equal number of agents with positive or negative food rewards (seed $4$, step $7.1e6$ to $7.2e6$). While lines indicate the trajectories, the circle indicates places where agents got their children. The size of circles corresponds to the number of grand children that the child born in that place made, indicating the influence of the reproduction event. In this period, agents with negative rewards for poor foods tend to producemany children in the right are of the environment with normal foods, leading to the population increase of the environment.
...
\Cref{figure:result-poison} shows the results with poisonous foods. Same as the case with poor foods, rewards for normal foods are positive in all runs. The rewards for poisonous foods are consistently negative in most runs, as expected, but there is a case with positive
