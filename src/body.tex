...
\begin{figure}[t]
  ...
  \caption{
    ...
    In the center and right figures, reward weights are colorized per food reward weight, as in \cref{sigure:s5-lh}
  }\label{figure:baseline-result}
\end{figure}

In the baseline environment, agents are densely located in the food location, which seems to influence the evolution of rewards. For example, negative rewards for food or another agent would help them move to sparser locations where they are more likely to produce offspring. Therefore, we conduct another experiment with more distributed food regeneration (\texttt{spread foods} in the figure) and moving food location after eating $1000$ foods (\texttt{food reloc.} in the figure).

\Cref{subfigure:blcomp} compares the evolved reward functions with the baseline. Although food rewards can be unstable with all these three settings, the \texttt{spread foods}
