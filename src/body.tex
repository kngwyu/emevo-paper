...
\section{Results}
As a baseline environment, we use the setting where food is always regenerated in the upper right of the room, as the left figure in \cref{figure:env} shows. All experiments start with $50$ agents that have energy level $40.0$. Initial reward weights are sampled from a zero-mean Gaussian distribution with standard deviation $0.1$, so that they should take values close to $0$. We conducted \num{1e7} step of the simulation, resulting in from $300$ to $400$ generations, which takes $14\sim16$ hours on NVIDIA P100 GPU.
...
\Cref{figure:result-baseline} shows the evolved reward weights with 5 random seeds in the baseline environment. We can observe that food reward is positive in all 5 runs, while action reward tends to be negative in the most of runs.

\begin{figure}[t]
  \centering
  \includegraphics[width=8cm]{comp_metabolism.png}
  \caption{Comparison of evolved rewards between different metabolic balance.}\label{figure:result-metabolism}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=8cm]{comp_foodloc.png}
  \caption{Comparison of evolved rewards between different metabolic balance.}\label{figure:result-metabolism}
\end{figure}
...
\begin{figure*}[t]
  ...
  \caption{
  }\label{figure:result-population}
\end{figure*}
