...
\section{Results}
As a baseline environment, we use the setting where food is always regenerated in the upper right of the room as the left figure in \cref{figure:env} shows, because we found this setting the easiest. In this environment, we tuned environmenal parameters (e.g., maxmimum number of foods and growth rate) so that the population is stable around $100$. We show all enviornmental parameters in \cref{ap:param}.

Our aim is to test what condition can affect the evolution of food reward.

For this purpose, we try the following 4 settings: \rnum{1}
