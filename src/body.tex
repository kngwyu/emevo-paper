...
\begin{figure}[t]
  ...
  \caption{
    ...
    Blue circles are agents, red circles are foods, outer gray lines are walls.
    Thin gray lines around agents are indicate distance sensors.
  }\label{figure:env}
\end{figure}
...
We assume that the reward function is determined at birth with mutation and does not change during an agent's lifetime. As inputs to the reward function, we use food intake and the magnitude of agent's action (motor output). We expect a positive reward for food evolves to acquire energy and a negative reward for action evolves to save energy.
